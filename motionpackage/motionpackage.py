...
#             with open(self.path, 'w') as f:
#                 config.write(f)
#             # self.LoadingWalkingGaitFunction(0)
#         elif msg.mode == 3:
#             self.walk_status = 3
#             self.path = f"{self.location}/Single_Parameter.ini"
#             config = configparser.ConfigParser()
...
# 若你只用 Node.get_logger()，其實不需要額外 import rclpy.logging
# import rclpy.logging
...
class Motionpackage(Node):
    def __init__(self):
        ...
        # -----------------------------
        self.declare_parameter('location', 'ar')
        self._apply_location(self.get_parameter('location').get_parameter_value().string_value)
        ...
        # 硬體/背景程序
        # -----------------------------
        self.serial_init()         # 你原本的初始化
        self.start_imu_thread()    # 你原本的初始化
        self._setup_gpio()
        self.standini()

        # 每 2 秒重試任何尚未連上的裝置，不會阻塞主執行緒
        self.reconnect_timer = self.create_timer(2.0, self._try_reconnect_serials)

        self.timer_dio = self.create_timer(0.1, self.dio)
        self.get_logger().info('motionpackage_node initialized ✅')
    ...
    def _try_reconnect_serials(self):
        """週期性嘗試把尚未連上的裝置打開。成功後記得初始化對應 Group/torque。"""
        # IMU
        if getattr(self, "serial_imu", None) is None:
            try:
                self.serial_imu = serial.Serial(self.port_imu, self.baudrate, timeout=1)
                time.sleep(1)
                self.get_logger().warn(f"[Reconnected] IMU on {self.port_imu}")
            except Exception:
                pass

        # WALK (OpenCR)
        if getattr(self, "serial_walk", None) is None:
            try:
                self.serial_walk = serial.Serial(self.port_walk, self.baudrate, timeout=0.1, rtscts=True)
                time.sleep(0.5)
                self.get_logger().warn(f"[Reconnected] WALK on {self.port_walk}")
            except Exception:
                pass

        # DXL 手部 (USB1)
        if getattr(self, "port_handler", None) is None:
            self._open_dxl_port(kind="hand")

        # DXL 腰/頭 (USB0)
        if getattr(self, "port_handler_1", None) is None:
            self._open_dxl_port(kind="waist")

        # 如果兩邊 DXL 都連上了而且還沒建 groupwrite，就建一下
        # （_open_dxl_port 已處理，但保險起見）

    def _open_dxl_port(self, kind: str):
        """kind: 'hand' 使用 self.port_head_dev；'waist' 使用 self.port_waist_dev"""
        if kind == "hand":
            dev = self.port_head_dev
            setattr_name_port = "port_handler"
            setattr_name_pkt  = "packet_handler"
            setattr_name_grp  = "groupwrite"
            ids = DXL_HAND_IDS
        else:
            dev = self.port_waist_dev
            setattr_name_port = "port_handler_1"
            setattr_name_pkt  = "packet_handler_1"
            setattr_name_grp  = "groupwrite_1"
            ids = DXL_HEAD_IDS
        ...
        try:
            ...
            if not ph.openPort():
                raise RuntimeError(f"openPort failed {dev}")
            if not ph.setBaudRate(self.baudrate_head):
                raise RuntimeError(f"setBaudRate failed {self.baudrate_head}")

            gw = GroupSyncWrite(ph, pk, 112, 8)  # ProfileVelocity(4) + GoalPosition(4)
            setattr(self, setattr_name_port, ph)
            setattr(self, setattr_name_pkt,  pk)
            setattr(self, setattr_name_grp,  gw)

            # 預啟用 torque（失敗就略過，不要 raise）
            for dxl_id in ids:
                res, err = pk.write1ByteTxRx(ph, dxl_id, ADDR_PRO_TORQUE_ENABLE, TORQUE_ENABLE)
                # 不阻斷，僅 debug 或 info
            self.get_logger().warn(f"[Reconnected] Dynamixel on {dev}")
        except Exception as e:
            # 保持 None 狀態，等下次重連
            setattr(self, setattr_name_port, None)
            setattr(self, setattr_name_pkt,  None)
            setattr(self, setattr_name_grp,  None)
            # 降級為 debug，避免洗版
            self.get_logger().debug(f"[DXL reconnect failed {dev}] {e}")


    # ====== Publishers ======
    def _create_publishers(self):
        self.sensor_data_pub = self.create_publisher(
            SensorPackage, '/package/sensorpackage', self.qos_sensor
        )
        # 建議用一致 QoS；不要混用 int 與 QoSProfile
        self.Dio_pub = self.create_publisher(
            Dio, '/package/FPGAack', self.qos_cmd
        )
        self.InterfaceCallback_Publish = self.create_publisher(
            Bool, '/package/motioncallback', self.qos_cmd
        )
        self.ExecuteCallBack_Publish = self.create_publisher(
            Bool, '/package/executecallback', self.qos_cmd
        )

    # ====== Subscriptions ======
    def _create_subscriptions(self):
        # 記得保留對 subscription 的引用，避免被 GC
        self.sensor_set_sub = self.create_subscription(
            SensorSet, '/sensorset', self.SensorSetFunction, self.qos_sensor
        )
        self.headmotor_sub = self.create_subscription(
            HeadPackage, '/package/HeadMotor', self.HeadMotorFunction, self.qos_cmd
        )
        self.InterfaceSend2Sector_sub = self.create_subscription(
            InterfaceSend2Sector, '/package/InterfaceSend2Sector',
            self.InterfaceSend2SectorFunction, self.qos_cmd
        )
        self.SectorSend2FPGA_sub = self.create_subscription(
            Int16, '/package/Sector', self.SectorSendOpenCRFunction, self.qos_cmd
        )
        self.InterfaceSaveData_sub = self.create_subscription(
            SaveMotion, '/package/InterfaceSaveMotion',
            self.InterfaceSaveDataDataFunction, self.qos_cmd
        )
        self.Continousback_sub = self.create_subscription(
            Bool, '/walkinggait/Continuousback', self.ContinousbackFunction, self.qos_cmd
        )
        self.ChangeContinuousValue_sub = self.create_subscription(
            Interface, '/ChangeContinuousValue_Topic',
            self.ChangeContinuousValueFunction, self.qos_cmd
        )
        self.SaveWalkingGaitParameter_sub = self.create_subscription(
            Parametermessage, '/web/parameter_Topic',
            self.SaveWalkingGaitFunction, self.qos_cmd
        )
        self.Gerente_sub = self.create_subscription(
            Int16, '/ContinousMode_Topic', self.GerenteFunction, self.qos_cmd
        )
        self.Send_sub = self.create_subscription(
            Bool, '/Send_parameter', self.SendtoOpenCR, self.qos_cmd
        )
        self.torque_sub = self.create_subscription(
            ButtonColorForm, '/ColorModelForm_Topic', self.Torque, self.qos_cmd
        )
        self.SingleMotor_sub = self.create_subscription(
            SingleMotorData, '/package/SingleMotorData',
            self.move_single_motor, self.qos_cmd
        )

    # ====== 分段封裝：Services ======
    def _create_services(self):
        self.InterfaceReadData_service = self.create_service(
            ReadMotion, '/package/InterfaceReadSaveMotion', self.InterfaceReadDataFunction
        )
        self.loadingwalkinggait_service = self.create_service(
            WalkingGaitParameter, '/web/LoadWalkingGaitParameter', self.LoadingWalkingGaitFunction
        )
        self.InterfaceCheckSectorFunction_service = self.create_service(
            CheckSector, '/package/InterfaceCheckSector', self.InterfaceCheckSectorFunction
        )

    # ====== 參數應用與動態更新 ======
    def _apply_location(self, loc: str):
        """將 location 參數套用到你的檔案路徑/內部狀態。"""
        try:
            # 你原本呼叫 location_callback(loc) 的設計像是接 ROS 訊息；
            # 改成內部方法：若你已有能吃字串的版本，可直接調用。
            self.location_callback(loc)  # 若你的函式只吃 msg，請改寫為可吃 str
            self.get_logger().info(f'Location set to: {loc}')
        except Exception as e:
            self.get_logger().error(f'Failed to apply location "{loc}": {e}')

    def _on_param_update(self, params):
        """rclpy 動態參數更新 callback。"""
        from rclpy.parameter import Parameter
        results = []
        for p in params:
            if p.name == 'location' and p.type_ == Parameter.Type.STRING:
                self._apply_location(p.value)
                results.append(p)
        # 回傳成功（若要更嚴謹可用 SetParametersResult）
    ...
    def _setup_gpio(self):
        GPIO.setmode(GPIO.BOARD)
        self.pins = [29, 31, 32, 33]   # 你的輸入腳位
        self.exponents = [1, 2, 3, 4]  # 你的邏輯運算用途
        for p in self.pins:
            GPIO.setup(p, GPIO.IN)
    ...
    def standini(self):
        self.RobotisListinit()
        self.get_logger().info("Standini")

        # 沒 Walk / 沒 DXL → 直接略過（node 繼續活著）
        if getattr(self, "serial_walk", None) is None:
            self.get_logger().error("[Standini] WALK serial not ready, skip sending to OpenCR")
            return
        if getattr(self, "packet_handler", None) is None or getattr(self, "groupwrite", None) is None:
            self.get_logger().error("[Standini] DXL(hand) not ready, skip hand init")
            # 不 return；你也許仍想發 OpenCR 的包，可以視需求決定

        try:
            handpkg, pkg16 = self.load_packets_from_toml()
        except Exception as e:
            self.get_logger().error(f"[Standini] load_packets_from_toml failed: {e}")
            return

        # 發給 OpenCR（已保證 serial_walk 存在）
        try:
            payload_vals = pkg16[1:-1]
            buf = bytearray([242])
            for val in payload_vals:
            ...
            self.get_logger().error(f"[Standini] WALK write error: {e}")

        # 發給手/腰（只有在 DXL 可用時才做）
        if getattr(self, "packet_handler", None) and getattr(self, "groupwrite", None):
            try:
                self.send_hand(handpkg)
            except Exception as e:
                self.get_logger().error(f"[Standini] send_hand failed: {e}")

        if getattr(self, "packet_handler_1", None) and getattr(self, "groupwrite_1", None):
            try:
                self.send_waist(handpkg)
            except Exception as e:
                self.get_logger().error(f"[Standini] send_waist failed: {e}")
